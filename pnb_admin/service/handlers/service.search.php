...
function service_search_handler($params) {
  ...
  if ( isset($params['field']) && ( $params['field'] == '0' || $params['field'] == 0 ) ) {
	  unset($params['field']);
  }
...
function search_institutions($field_id, $keyword, $type, $autocomplete) {
	...
	switch ($type) {
		...
		case 'soundex':
			...
			break;
		case 'empty':
			$query = 'SELECT m.id as institutions_id, "" as value FROM `'.$db_name.'`.`institutions` m WHERE m.status = "active" AND NOT EXISTS( SELECT * FROM `'.$db_name.'`.`institutions_data_strings` mds WHERE mds.institutions_fields_id = '.$field_id.' AND mds.institutions_id = m.id )';
			$res = $db->Query($query);
			break;
		case 'equals':
		case 'default':
